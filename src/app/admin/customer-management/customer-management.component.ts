...
import { DeleteConfirmationDialog } from '../../app.component'; // Adjust the path as necessary
...
export class CustomerManagementComponent {
  ...
  limit: number = 10;
  offset: number = 0;
  cusDetails:boolean = false
  date2: Date | undefined;
  rejectvisible :boolean = false
  rejectId: any = null;
  totalCustomer: number = 0;
  approvedCustomer: number = 0;
  rejectedCustomer: number = 0;
  ...
  //   this.adminService.get('/customerDet').subscribe((res) => {
  //     // setTimeout(() => {
  ...
  onSearch() {
    this.filteredProducts = []
    let apiUrl = '/customerSearch';
    this.adminService.getCutomer(apiUrl,this.searchQuery, this.limit, this.offset).subscribe({
        next: (res:any) => {
          this.filteredProducts = res.result;
        },
        error: (error) => {
...
getDashboardData() {
  this.adminService.get('/dashboardGet').subscribe((res) => {
    if (res.dataStatus) {
      // Extracting Juristic data
    

      // Extracting Customer data
      const customerData = res.result.customerDashboard[0] || {};
      this.totalCustomer = customerData.total || 0;
      this.approvedCustomer = customerData.approved || 0;
      this.rejectedCustomer = customerData.rejected || 0;

      console.log('Customer Dashboard', customerData);
    } else {
