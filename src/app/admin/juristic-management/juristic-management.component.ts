...
@Component({
  selector: 'app-juristic-management',
  templateUrl: './juristic-management.component.html',
  styleUrl: './juristic-management.component.scss'
})
export class JuristicManagementComponent {
  ...
  rejectId: any = null;
  totalJuristic: number = 0;
  approvedJuristic: number = 0;
  rejectedJuristic: number = 0;
  ...
  initValidators() {
    ...
    this.KYCDocuments = this.fb.group({
      ...
      nationalid_upload: ['', Validators.required],
  ...
  onSaveAndNext() {
    ...
    if (this.activeIndex === 0) {
      ...
      this.proffesionalInfo_submitted = true;
      if (this.proffesionalInfo.valid) {
        this.activeIndex = 2;
      } else {
        this.proffesionalInfo.markAllAsTouched();
      }
    } else if (this.activeIndex === 2) {
      this.KYCDocuments_submitted = true;
      if (this.KYCDocuments.valid) {
        if (!this.selectedCustomer?.id) {
          this.activeIndex = 3;
          this.openTermsModal();
        }
  ...
  submitForm() {
    ...
    let data = {
      records: {
        ...
        nationalid: this.KYCDocuments.value.nationalid,
        nationalid_upload: this.KYCDocuments.value.nationalid_upload,
...
removeDoc(form: string, field: string) {
  ...
  onSearch() {
    let apiUrl = '/juristicSearch';
    this.adminservice.gets(apiUrl,this.searchQuery, this.limit, this.offset).subscribe({
        next: (res:any) => {
          this.products = res.result;
          this.filteredProducts = this.products;
        },
...
getDashboardData() {
  this.adminservice.get('/dashboardGet').subscribe((res) => {
    if (res.dataStatus) {
      const juristicData = res.result.juristicDashboard[0] || {};
      
      // Assigning the values from juristic dashboard
      this.totalJuristic = juristicData.total || 0;
      this.approvedJuristic = juristicData.approved || 0;
      this.rejectedJuristic = juristicData.rejected || 0;

      console.log('Juristic Dashboard', juristicData);
    } else {
