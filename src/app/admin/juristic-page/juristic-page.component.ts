...
@Component({
  selector: 'app-juristic-page',
  templateUrl: './juristic-page.component.html',
  styleUrl: './juristic-page.component.scss'
})
export class JuristicPageComponent {


  constructor( private _location: Location, private adminService:AdminService,private route: ActivatedRoute, private fb: FormBuilder,
  ...
  isLoan:boolean = true
  customer_id :any
  showFormCredit : boolean = false
  credit_submitted :boolean = false
  ...
  onSaveAndNextCredit() {
    if (this.selectedCustomer?.id) {
      this.submitFormCredit();
    }
    if (this.activeIndex === 0) {
      this.credit_submitted = true;
  ...
  getlaonData(){
    this.customer_id =  localStorage.getItem('loginId')
  }

  getData(id: string) {
    this.adminService.getCustomerData('/juristicPerDetbyId', id).subscribe((res) => {
