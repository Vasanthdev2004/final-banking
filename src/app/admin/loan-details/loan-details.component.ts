...
export class LoanDetailsComponent {
  ...
  editJuristicPerson(product: any): void {
    ...
    // this.showForm = true;
    // Implement logic to edit the juristic person
  }
  ...
  approve() {
    ...
    this.adminService.post(apiUrl, data).subscribe(res => {
      this,this.approvevisible = false
      this.getCustomerData()
      this.isButton = false
      if (res.dataStatus) {
        this.toastr.success(res.message, 'Success');
  ...
  reject() {
      ...
      Swal.fire({
        ...
        preConfirm: () => {
          ...
          if (!termsCheckbox || !(termsCheckbox as HTMLInputElement).checked) {
            Swal.showValidationMessage('You need to agree to the Terms and Conditions');
            return false;
          }
          return true;
        }
    ...
    onSearch() {
      let apiUrl = '/loanSearch';
      this.adminService.getLaon(apiUrl,this.searchQuery, this.limit, this.offset).subscribe({
          next: (res:any) => {
            this.products = res.result;
            this.filteredProducts = this.products;
          },
